...
Write-Host "=== 2/4 creating VM ($size, Win2022) ==="
...
Write-Host "=== 3/4 installing VS Code + Azure CLI on the VM ==="
$script = @'
$ErrorActionPreference="Stop";
Invoke-WebRequest "https://update.code.visualstudio.com/latest/win32-x64/stable" -OutFile "$env:TEMP\vscode.exe";
Start-Process "$env:TEMP\vscode.exe" -Args "/VERYSILENT /MERGETASKS=!runcode,addtopath" -Wait;
Invoke-WebRequest "https://aka.ms/installazurecliwindows" -OutFile "$env:TEMP\azcli.msi";
Start-Process msiexec.exe -Args "/i `"$env:TEMP\azcli.msi`" /qn" -Wait;
"installed" | Out-File C:\lab513-bootstrap.txt
'@
az vm run-command invoke -g $rg -n $vm --command-id RunPowerShellScript --scripts $script -o none
